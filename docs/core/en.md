...
| `Version` | `long` | Count of the structural changes applied so far (monotonic, lock-free to read): anything you derived from `Roots` can be invalidated on a change — only *that* it changed is meaningful, not the starting value |
| `PendingChangeCount` | `int` | Structural changes other threads queued and nobody has applied yet (diagnostics: it keeps climbing while the frame boundary is not running) |
| `MaxPendingChanges` | `int` | Cap on the producer queue (defaults to `DefaultMaxPendingChanges` = 65536, 0 = unbounded): past it, further changes are dropped with one warning. Not a performance knob — it makes misuse visible, because a producer enqueueing structure at data rate would otherwise climb to memory exhaustion, which is far harder to diagnose than one warning |
| `DroppedPendingChangeCount` | `long` | How many changes were dropped for exceeding `MaxPendingChanges` (diagnostics; always 0 in healthy use) |
| `IsOwnerThread` | `bool` | Whether the calling thread is the scene's owner — the one allowed to walk `Roots` / `Lights` and run `Update` / `ApplyPendingChanges`, and the one on which `Add` / `Remove` apply immediately |

Methods:
- `void Add(GameObject? obj)` — add to the scene (root nodes are registered automatically; a `Light` also joins `Lights`). **Any thread may call it**: on the scene's owner thread (the constructing thread, or the frame-loop thread a host handed the scene over to with `ClaimOwnership()` before the loop started) the object joins immediately; on any other thread it is queued and joins at the next frame boundary. That is what makes "keep building the scene while it renders" safe — the queue gathers structural changes at the boundary, so a render traversal can never meet a list that a producer is editing. Once applied the object behaves exactly like one that was there from the start: the renderer creates GPU resources **lazily** (on the first frame it sees the node) and walks `Roots` again every frame. An object that has a parent is not a root: adding it registers nothing new (it is drawn through that node), and a root that later gains a parent leaves `Roots` again — so one node never sits in `Roots` *and* under a parent
- `void Remove(GameObject? obj)` — remove from the scene (a `Light` also leaves `Lights`); the threading contract is `Add`'s. Both the root list and the parent are cleared (not just whichever one the node's `Parent` points at), which makes **removing idempotent**: one `Remove` is enough whatever state the node is in, and "gone only after a second Remove" cannot happen. What leaves cannot stay selected: removing the selected node, or an ancestor of it (which takes the whole subtree out), also drops the selection's highlight and the axes `Select` mounted
- `int ApplyPendingChanges()` — the **frame boundary**: applies the structural changes other threads queued (`Add` / `Remove` / re-parenting) and returns how many it applied. `Update` and the renderer (any `IRenderer.Render` implementation) call it for you first, so the usual frame loop needs no extra call (the frame's second call finds the queue empty and returns without taking the lock); a host that drives its own rendering path can call it itself. It must run on the **owner thread**, and it **no longer makes the calling thread the owner**: on any other thread it throws `InvalidOperationException` pointing at `ClaimOwnership()`, because "the thread allowed to walk `Roots`" and "the thread allowed to write the lists" have to stay one thread — a silent reroute turns the previous owner (possibly mid-traversal) into an outsider. After `Dispose` it only drains the queue, never applies it: a frame boundary does not throw because a producer arrived late
- `void ClaimOwnership()` — hand the scene over to the calling thread explicitly (once, **before the loop starts**): that thread becomes the owner, so `Add` / `Remove` apply immediately on it and `Update` / `ApplyPendingChanges` are allowed on it. For a host that builds the scene in one place and runs the frame loop on another; a host that constructs its scene inside (or on the thread of) its frame loop never needs it. Handing over after a frame boundary has run throws — the current owner may be walking the lists, and a second owner would be editing them underneath it. Calling it on the owner thread is a no-op, so a host can call it defensively
- `Update(double deltaTime)` — crosses the frame boundary (`ApplyPendingChanges`) first, then recurses into each node `Update` (CPU data only, never GL). Since `Update` and `Render` are both frame boundaries, both belong to the **same thread** (the host's frame loop) — the scene's owner thread
- Re-parenting the tree: a `Transform.Parent` assignment is the only way in (`Transform.Children` is a read-only view). Once the node is in a scene the assignment goes through the scene's frame boundary: immediate on the owner thread, keeping `Roots` membership in step; queued on any other thread, visible by the next frame at the latest. Before it joins a scene a node belongs to none, so building a subtree on any thread stays immediate (assembling the subtree *before* `Add` is the recommended shape anyway). Assigning a parent that would close a cycle (the node itself or one of its descendants) throws `ArgumentException`; when such a change came from another thread and only turns out to be a cycle at the boundary, it is dropped with one warning — the frame loop is not taken down over a request that was valid when it was made
...
### `Light`
`enum LightType { Directional, Point }`. Members: `Type`, `Color` (`Vector3` intensity), `Position` / `Direction` (both in the node's own frame, like every other `GameObject`), `Intensity`, `Range`. The renderer reads the world-space pair `WorldPosition` / `WorldDirection` (position composed with the ancestor chain, direction carried through it and re-normalized), so a light parented to something — a lamp on a robot — lights from where it really is; an invisible light (`Visible = false`) is left out of the shading entirely. `Range` is carried but not consumed yet: the standard shading has no attenuation term.
...
| walking `SceneGraph.Roots` / `Lights` from a non-owner thread, or expecting a non-owner thread's `Add` to show up in them immediately | structural changes commit at a frame boundary: walking and reading belong to the owner thread (the render thread), producers only enqueue, and the boundary is when it shows up |
| putting `SceneGraph.Update` and `Renderer.Render` on two different threads | both are frame boundaries (both fold in queued structural changes), so two threads means two commits racing each other |
