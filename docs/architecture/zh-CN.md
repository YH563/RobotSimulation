...
1. **单一依赖方向**：`Host → OpenGL/Robot → Core`，禁止反向引用。
...
```text
┌─────────────────────────────────────────────────────────────────┐
│  (sample) RobotSimulation.Host  — exe, 窗口/输入, 组装根          │
...
3. 所有 `Silk.NET.*` / `StbImageSharp` 只在 `OpenGL`（及示例宿主窗口/输入）出现，永不出现在 public 签名。
...
| ADR-011 | 宿主经 `OpenGL/Device/GraphicsFactory` 组装后端（接口化组合根） | Host 只持有 `IRenderContext` / `IRenderer` |
...
| ADR-018 | 通过组合根（宿主）提供 `GL`，后端以接口交付；宿主仅持接口与视口尺寸 | `Host` 可整体切换（裸窗口 / WPF / Avalonia） |
...
4. **测试宿主（三宿主测试项目）**：裸窗口、WPF、Avalonia 三个宿主示例/测试项目——每个内置一个 `RobotViewport` 式控件，既证明本库**能在不同桌面框架下独立渲染显示**，也把相关测试收编进去；仅作验证，不随库发布。它们共享同一 `Core`/`Robot`/`OpenGL` 链路，宿主差异只在「如何拿 `GL` 与如何同步视口」。
