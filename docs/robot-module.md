...
```
RobotSimulation.Robot                  （单入口：RobotModel = 机器人 GameObject 树根）
    │
    ...
    │                           Core.Geometry/Scene/Rendering（仅 CPU 数据资源，构建机器人树）
...
│      public sealed class RobotModel : GameObject   ← 机器人 = 一棵 GameObject 树（根 link 骨架即它本身）
...
│          RobotModel(RobotDescription desc, IAssetResolver? = null)  // 扩展点：任意描述源 → 树
│          RobotDescription Description { get; }        // 纯数据描述（headless/序列化/FK/多实例复用）
│          RobotState CreateState()                      //    便捷 headless 状态
...
│              // 规则：绝对路径按绝对找；相对路径以 URDF 同级目录合并；package:// 剥前缀后在 URDF 同级查找
│              // 找不到文件直接抛 FileNotFoundException（Logger 记录），不做静默跳过
│
└── State/                              命名空间 RobotSimulation.Robot.State     [M3 ✅]
    └── RobotState.cs                    public sealed class RobotState（headless FK + 关节值）
        ...
        纯计算、零渲染依赖；可视化驱动另由 Robot/RobotModel 完成
```

## 3. 可见性与公开面（对齐 api.md）

- **public（用户面）**：`RobotModel`、`IAssetResolver`、`FileSystemAssetResolver`、
  `UrdfParseException`、全部 `Description` 数据模型。
- **internal（实现细节）**：`UrdfParser`——用户只通过 `RobotModel.Parse/ParseFile` 进入；
  单测同样覆盖该入口。
- Robot 层不再出现任何渲染/GL 资源：`RobotModel` 构建（`Parse`/`new RobotModel(desc)`）只产出 CPU 数据（MeshData/MaterialData）；
...
6. **Mesh 引用不在此层解析文件**：`MeshGeometry.Uri` 仅存原始字符串，由 Core/Geometry/Import（M2.5）
   在数据导入阶段经 `IAssetResolver` 消费，产物是 `MeshData`（随后由 RobotModel 构建时挂到 visual 节点）。
...
3. ✅ `Robot/RobotModel.cs`（`RobotModel : GameObject`：读 URDF/任意描述 → 完整机器人树 + 关节驱动）；
...
RobotModel.ParseFile("x.urdf")
   ...
   → [M3] RobotModel.ParseFile(...)（RobotModel : GameObject 树根；scene.Add 后由 Renderer 绘制）
